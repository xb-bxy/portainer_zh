...
export function DashboardItem({
  ...
  let displayValue;
  if (isChinese) {
    displayValue = translatedType;
  } else if (typeof value === 'number') {
    displayValue = pluralize(value, translatedType, pluralType);
  } else {
    displayValue = translatedType;
  }
  ...
  const Item = (
    ...
    >
      ...
      >
        {t('dashboardItem.refreshingTotal')}
      ...
      >
        {t('dashboardItem.loadingTotal')}
        <Loader2 className="h-4 animate-spin-slow" />
      </div>
      <div className="flex items-center" aria-label={translatedType}>
        <div
          className={clsx(
