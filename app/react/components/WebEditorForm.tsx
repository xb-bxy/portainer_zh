...
function DefaultTitle({ id }: { id: string }) {
  ...
  const translatedEditorConfig = Object.entries(editorConfig).reduce(
    (acc, [key, config]) => {
      acc[key as keyof typeof editorConfig] = {
        ...config,
        searchCmdLabel: t(config.searchCmdLabel),
        tooltip: {
          ...config.tooltip,
          props: {
            ...config.tooltip.props,
            children: config.tooltip.props.children.map(
              (child: (typeof config.tooltip.props.children)[0]) => ({
                ...child,
                props: {
                  ...child.props,
    ...
    {} as Record<
      keyof typeof editorConfig,
      (typeof editorConfig)[keyof typeof editorConfig]
    >
  );
  ...
  return (
    <FormSectionTitle htmlFor={id}>
      ...
      <div className="text-muted small vertical-center ml-auto">
        {translatedEditorConfig[BROWSER_OS_PLATFORM].searchCmdLabel}

        <Tooltip
          message={translatedEditorConfig[BROWSER_OS_PLATFORM].tooltip}
        />
      </div>
    </FormSectionTitle>
  );
