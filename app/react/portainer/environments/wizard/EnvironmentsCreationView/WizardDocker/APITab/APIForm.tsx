    ...
    >
      {({ values, errors, setFieldValue, isValid, dirty }) => (
        <Form>
          ...
          <FormControl
            inputId="url-field"
            label={t('apiForm.label.dockerApiUrl', 'Docker API URL')}
            required
            tooltip={t(
              'apiForm.tooltip.dockerApiUrl',
          ...
          >
            <Field
              ...
              name="url"
              placeholder={t(
                'apiForm.placeholder.dockerApiUrl',
                'e.g. 10.0.0.10:2375 or mydocker.mydomain.com:2375'
              )}
            />
          </FormControl>
          ...
          <div className="form-group">
            <div className="col-sm-12">
              <LoadingButton
                ...
                icon={Plug2}
              >
                {t('apiForm.button.connect', 'Connect')}
              </LoadingButton>
            </div>
          </div>
        </Form>
    ...
    mutation.mutate(
      ...
      },
      {
        onSuccess(environment) {
          notifySuccess(
            t('apiForm.success.title', 'Environment created'),
            environment.Name
          );
          clearForm();
