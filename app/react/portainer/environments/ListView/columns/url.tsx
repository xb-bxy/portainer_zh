...
function Cell({
          ...
          environment.StatusMessage?.detail && (
            <div className="ml-2 inline-block">
              <span className="text-danger vertical-center inline-flex">
                ...
                <span>{t(environment.StatusMessage.summary)}</span>
              </span>
              <TooltipWithChildren
                message={
                  <div>
                    {t(environment.StatusMessage.detail)}
                    {environment.URL && (
                      <div className="mt-2 text-right">
                        ...
                        >
                          <span className="text-muted-light">
                            {t('Dismiss error (still visible in logs)')}
                          </span>
                        </Button>
                      </div>
                    )}
  ...
  if (environment.Status === EnvironmentStatus.Provisioning) {
    const summary = environment.StatusMessage?.summary;
    return (
